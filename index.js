...
async function run() {
  try {
    const usersCollection = client.db('ContestCreation').collection('users');
    ...
    // Get a single contest data from db using _id
...
app.get('/', (req, res) => {
  res.send('Hello from Contest Creation Platform Server..')
})

app.listen(port, () => {
  console.log(`Contest Creation Platform is running on port ${port}`)
})
